...
def evaluate(model, loader, device):
    ...
    Returns:
        results (dict): 预测结果
        auc (float): ROC AUC 分数
        acc (float): 准确率
    """
    n_classes = model.n_classes  # 确保模型有 n_classes 属性
    patient_results, test_error, auc, acc_logger = summary(model, loader, n_classes)
    ...
    return patient_results, auc, acc
